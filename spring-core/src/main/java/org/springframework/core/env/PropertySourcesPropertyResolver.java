...
public class PropertySourcesPropertyResolver extends AbstractPropertyResolver {
	 ...
	 *
	 * 属性获取方法
	 *	PropertySourcePropertyResolver通过
	 */
	protected <T> T getProperty(String key, Class<T> targetValueType, boolean resolveNestedPlaceholders) {
		if (this.propertySources != null) {
			...
			/**
			 * 循环这个数据结构（List的数据结构），
			 * 从第一个PropertySource开始循环， 寻找对应的name的对应的属性值，如果找到了就返回
			 *	不会在继续往下寻找
			 *	每个PropertySource是一个存储属性的数据结构，比如MapPropertySource是一个Map,
			 *		PropertiesPropertySource是一个Properties，JndiPropertySource等等
			 *	参数泛型都是一个数据结构，等等。
			 */
