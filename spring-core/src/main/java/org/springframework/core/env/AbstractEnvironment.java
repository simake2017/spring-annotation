...
/**
	基础的抽象实现类
 	一个是PropertySources，一个是 以 Profiles，
	这个类已经是比较完整的实现了

 	这个类是Environment的基础实现类
...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	/**
	 *	环境里面有MutablePropertySources类
	 *	这里是属性资源集合 ， 是一个数据结构，存放的是propertySource
	 *
	  * */
	...
	/**
	 * 环境也实现了PropertyResolver接口，但是他的实现是依靠一个PropertyResolver的具体子类PropertySourcesPropertyResolver
	 * 来实现的，不是依靠他自身提供过的方法来实现的。
	 *
	 * 解析占位符的来源为propertySources这个list结构
	 *
	 * */

	/**
	 * 这里是属性解析，从类似于List<PropertySource>的数据结构中解析相应的属性
	 *
	 */
	...
	/**
	 * 	是通过PropertyResolver工具类 从MutablePropertySources中解析出来的
     *	MutablePropertySources是一个list数据结构，里面的PropertySource是一个
	 *	property source数据结构，Source是一个类似于Map的结构
	 *	或者是Properties等可以存储key - value 键值对的信息的结构
	 */
	@Override
