 ...
 * xml定义的bean的读取
 *
 *  定制化的 Xml  BeanDefinitionReader ，用于读取xml资源 并生成BeanDefinition然后注册到
...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	/**
	 * 读取document文档转为 BeanDefinition
	 *
	 * */
	//Bean Definition 文档解析
	...
	/**
	 * resourceLoader用于读取xml文件
	 */

	/**
	 * 如果执行XmlBeanDefinitionReader，则需要指定一个registry,
	 * registry与resourceloader都会指定，如果是建立一个
	 * xmlBeanFactory则不会创建resourceLoader
	 *
	 * @return
	 */
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			...
			try {
				/**
				 * 使用sax方式解析inputStream文件，该文件在org.xml.sax包中，
				 * InputSource处理文件较快，在jdk包中
				 */
				/**
				 * inputSource 类 在 org.xml.sax包中，
				 */
				InputSource inputSource = new InputSource(inputStream);
				if (encodedResource.getEncoding() != null) {
					inputSource.setEncoding(encodedResource.getEncoding());//设置解码方式
				}
				//真正的解析的地方
			...
			currentResources.remove(encodedResource); //最后移除掉
	...
	/**
	 * documentLoader用于将resource解析并转为document
	 *
	 */
	...
	/*
	 *	根据document文档解析获取 BeanDefinition(bean定义)
	 *
	 */
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();

		//registry 也就是 XmlBeanFactory Docu
	...
	/*
		...
		在创建     Context 的时候     使用的  registry 是  XmlBeanDefinitionReader的 registry
	 */
