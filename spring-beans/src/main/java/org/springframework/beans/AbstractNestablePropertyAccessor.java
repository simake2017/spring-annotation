...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	protected AbstractNestablePropertyAccessor getPropertyAccessorForPropertyPath(String propertyPath) {
		int pos = PropertyAccessorUtils.getFirstNestedPropertySeparatorIndex(propertyPath); //获取 属性路径 分隔符
		...
		if (pos > -1) {
			 ...
			 * 这里会形成嵌套 关系
			 * 比如 a.b.c.userName
			 * 那么会先找到 第一个属性a 然后path: b.c.userName
			 * 然后 第二个属性b 然后path c.userName
			 * 然后 第三个属性c 然后path userName
			 * 这样递归下去  直到 返回this 对象
			 *
			 */

			String nestedProperty = propertyPath.substring(0, pos); // 这里是获取属性名称
			String nestedPath = propertyPath.substring(pos + 1); // 这里是 获取属性路径
	...
	private PropertyTokenHolder getPropertyNameTokens(String propertyName) {
		...
		/**
		 * 这里会相当于 a.b[0].userName 会形成这样的 一种 路径分隔
		 * 这里的属性间隔 是 一层一层形成的，keys里面通常只有一个值
		 *
		 */
		if (!keys.isEmpty()) {
